...
class PreProcessingFrame(tk.Frame):
    ...
    def setup_tool(self, frame):
        ...
        row2 = tk.Frame(frame)
        row2.pack(fill='x', expand=1)
        self.scale_value_max = tk.StringVar(value='100.0')
    ...
    def import_data(self):
        ...
        if data_type == 'mib':
            ...
            from read_empad import read_empad
            data_4d = read_empad(self.path, EMPAD_shape=(x,y,130,128))
        elif data_type == "tif":
        ...
        #self.center_fig = data_4d[x, y,:,:] # assign first frame for center finding
    ...
    def auto_find_center(self, fig):
        ...
        if fig is not None:
            ...
            #if self.path[-3:] == 'mib':
            MaxValue = float(self.scale_value_entry_max.get())/100
            MaxValue = np.max(fig)*MaxValue
            fig[fig>MaxValue] = MaxValue
            # fig_thresholded = np.array([[min(value, MaxValue) for value in row]
            #                             for row in fig])
            y, x = RDF_preparation.mean_center(fig)#, self.radius)
    ...
    def find_center(self, mode='given'):
        if mode == 'auto':
            x, y, radius = self.auto_find_center(self.center_fig.copy())
            self.radius = radius
    ...
    def calc_center_map(self):
        ...
        if data_4d is not None:
            self.center_map = RDF_preparation.CoM_Map(data_4d, MaxValue)
            ...
            self.plot_map()
            print('Center Mapping Calculation Done')
...
class RdfMapFrame(tk.Frame):
    ...
    def save_rdf_data(self):
        ...
        path2 = path[:-4]+'_StructureFactor_'+path[-4:] # for structure factor map
