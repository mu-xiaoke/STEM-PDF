...
class PreProcessingFrame(tk.Frame):
    ...
    def plot_map(self, event=None, max=0, min=0, mode=0):
        ...
        global data_4d
        # sum over real space to display avraged diffraction pattern
    ...
    def save_center_map(self):
        ...
        if self.center_map is not None:
            path = asksaveasfilename(filetypes=(('Numpy File', '*.npy'),
                                               ("All Files", "*.*")),
                                    title="Choose a file.")
            with open(path, 'wb+') as file:
    ...
    def import_data(self):
        ...
        if data_type == 'mib':
            ...
            self.row_data = np.load(self.path, mmap_mode='r')
            a = time.time()
        #elif data_type == "spy":
            #import hyperspy.api as hs
        elif data_type == "tif":
            ...
            import h5py
            self.row_data = h5py.File(self.path)['entry']['data']['data']
        else:
        #   self.row_data = sio.loadmat(self.path)['cube']
            f = hs.load(self.path)
            self.row_data = f.data
            global data_4d
            if len(self.row_data.shape) < 4:
                 pic_shape = np.shape(self.row_data[0]) #size of diffraction pattern
                 data_4d = self.row_data[0:x*y].reshape((x, y, pic_shape[0], pic_shape[1])) # 3d array to 4d
                 self.center_fig = self.row_data[0] # assign first frame for center finding
            elif len(self.row_data.shape) == 4:
                data_4d = f.data
                self.center_fig = data_4d[0,0]
    ...
    def auto_find_center(self, fig):
        ...
        if fig is not None:
            ...
            #if self.path[-3:] == 'mib':
            #x, y = RDF_preparation.mean_center(fig)
...
class ProfileMapFrame(tk.Frame):
    ...
    def calculate_profile_map(self, process_area=None):
        ...
        # Can be cestricted to process_area and uses center_map if present
        a=time.time()
        if self.center_map is not None:
            if self.center_map.shape[1:3] == data_4d.shape[0:2]:
