...
class MainGui(tk.Tk):
    ...
    def profile_map(self):
        ...
        if self.replot_profile:
            self.frame[ProfileMapFrame].plot_map()
        print('profile map')
...
class PreProcessingFrame(tk.Frame):
    ...
    def plot_map(self, event=None, max=0, min=0, mode=0):
        ...
        temp = np.sum(np.sum(self.whole_data[_position1[1]:_position2[1] + 1,
    ...
    def import_data(self):
        ...
        temp_path = askopenfilename(filetypes=(('Merlin Image Binary File', '*.mib'),
        ...
        if data_type == 'mib':
            self.row_data_address = read_mib.MIBFile(self.path)
            a = time.time()
            self.row_data = np.array(self.row_data_address._frames(x * y, 0)) #read the 3d array from data file
        elif data_type == "npy":
            self.row_data = np.load(self.path)
            a = time.time()
        elif data_type == "spy":
            #import hyperspy.api as hs
            f = hs.load(self.path)
            self.row_data = f.data
        elif data_type == "tif":
            self.row_data = np.array(Image.open(self.path))
        #else:
        #    self.row_data = sio.loadmat(self.path)['cube']

        pic_shape = np.shape(self.row_data[0]) #size of diffraction pattern
        self.whole_data = self.row_data[0:x*y].reshape((x, y, pic_shape[0], pic_shape[1])) # 3d array to 4d
        self.center_fig = self.row_data[0]
        try:
            x, y, radius = RDF_preparation.test_hough(self.center_fig)
            self.fig_map = np.sum(self.whole_data[:, :, x, :], 2) + \
                           np.sum(self.whole_data[:, :, y, :], 2)
            self.test_center = [x, y]
        except:
            self.fig_map = np.sum(self.whole_data[:, :, pic_shape[1] // 2, :], 2) + \
                           np.sum(self.whole_data[:, :, pic_shape[2] // 2, :], 2)
    ...
    def auto_find_center(self, fig):
        ...
        if fig is not None:
            x, y, radius = RDF_preparation.test_hough(fig)
            #if self.path[-3:] == 'mib':
            #x, y , radius= RDF_preparation.mean_center(fig)
            return x, y, radius
    ...
    def calculate_profile(self):
        ...
        self.update_plot(profile, title='profile')
...
class RdfFrame(tk.Frame):
    ...
    def setupGUI(self, frame):
        ...
        tk.Button(row_output, text='save 1d\n structure\n factor', width=8, height=4,
                  command=self.save_prisine_data).pack(side=tk.RIGHT)
    ...
    def save_rdf_data(self):
        ...
        with open(path, 'w') as f:
            ...
            if self.plot_dict['r'] is not None and self.with_x_int.get():
                np.savetxt(f, np.stack((self.plot_dict['r'], rdf_data), axis=-1), delimiter=' ',
                           fmt=data_format)
            else:
                np.savetxt(f, rdf_data, delimiter=' ', fmt=data_format)

    def save_prisine_data(self):
        data_format = '%.4e'
        path = asksaveasfilename(filetypes=(("text file", "*.txt"), ("All Files", "*.*")))
        with open(path, 'w') as f:
...
class ProfileMapFrame(tk.Frame):
    ...
    def setup_command_window(self, frame):
        ...
        #self.entry_center_x.pack(side=tk.LEFT, fill='x', expand=1)
    ...
    def save_profile_data(self):
        ...
        #if path[-3:] == 'csv':
        #    with open(path, 'wb+') as file:
        #        pickle.dump(self._profile_map, file)
        if path[-3:] == 'npy':
            with open(path, 'wb+') as file:
    ...
    def get_4d_data(self, path, mode, x, y):
        if mode == 'mib':
            mib_data = read_mib.MIBFile(path)
            _4d_data = mib_data._frames(int(x * y), 0)
            _4d_data = np.array(_4d_data).reshape((x,
                                                   y,
                                                   np.shape(_4d_data)[-2],
                                                   np.shape(_4d_data)[-1]))
        if mode == 'tif':
            tif_data = Image.open(path)[0:x*y]
            data_shape = np.shape(tif_data)
            _4d_data = np.reshape(tif_data, [x, y, data_shape[-2], data_shape[-1]])
        if mode == 'npy':
            _4d_data = np.load(path,mmap_mode='r')[0:x*y]
            data_shape = _4d_data.shape
            _4d_data = np.array(_4d_data).reshape([x,y,data_shape[-2],data_shape[-1]])
        if mode == 'spy':
            f = hs.load(path)
            _4d_data = f.data[0:x*y]
            data_shape = _4d_data.shape
            _4d_data = _4d_data.reshape([x, y, data_shape[-2], data_shape[-1]])
        return _4d_data
    ...
    def calculate_profile_map(self, process_area=None):
        ...
        try:
            ...
            print('invalid shape')
            return
        print(x, y, shape_x, shape_y)
        if self.path is not None:
            self._4d_data = self.get_4d_data(self.path, self.mode, shape_x, shape_y)
        else:
            return
        center = [y+1, x+1]
        if self.delete_mask is None:
            profile_test = RDF_preparation.intensity_average(self._4d_data[0, 0, :, :], center)
            profile_length = np.size(profile_test)
        else:
            profile_test = RDF_preparation.intensity_average(np.multiply(self._4d_data[0, 0, :, :],
                                                                         self.delete_mask), center)
            profile_length = np.size(profile_test)
        x_start = 0
        y_start = 0
        x_end = shape_x
        ...
        for i in np.arange(x_start, x_end):
            for j in np.arange(y_start, y_end):
                    single_pattern = self._4d_data[i, j, :, :]
                    ...
                    _profile_map[i-x_start,
                                 j-y_start,
                                 :] = RDF_preparation.intensity_average(single_pattern, center)
...
class RdfMapFrame(tk.Frame):
    ...
    def save_rdf_data(self):
        data_format = '%.4e'
        path = asksaveasfilename(filetypes=(("csv File", "*.csv"),
                                            ("npy File", "*.npy"),
                                            ("text File", "*.txt"),
                                            ("All Files", "*.*")))
        data_shape = np.shape(self._rdf_map)
        datainfo = str(data_shape[2]) + '_' + str(data_shape[0]) + '_' + str(data_shape[1])
        path = path[:-4] + '_size_' + datainfo + path[-4:]
        if path[-3:] == 'csv':
            with open(path, 'wb+') as file:
                pickle.dump(self._rdf_map, file)
                print("test pickle")
        if path[-3:] == 'npy':
            with open(path, 'wb+') as file:
                np.save(file,self._rdf_map,allow_pickle=False)
        elif path[-3:] == 'txt':
            with open(path, 'wb+') as file:
                data = self._rdf_map.reshape(np.size(self._rdf_map))
                data[np.isnan(data)] = 0
                np.savetxt(file, data, delimiter=",", newline="\r\n", fmt=data_format)

            path2 = path[:-4]+'_RIF_'+path[-4:]
            with open(path2, 'wb+') as file:
                data = self._RIF_map.reshape(np.size(self._RIF_map))
                data[np.isnan(data)] = 0
                np.savetxt(file, data, delimiter=",", newline="\r\n", fmt=data_format)
