...
def distance_matrix(fig, center):
    ...
    distance_mat = np.array(distance_mat + 0.5, dtype=int)
    '''
    a = Figure()
...
def auto_beam_stopper(image, initial_point, alpha_low=0.05, alpha_high=0.9):
    if image is None:
        return None, None
    timer = time.time()
    fig = np.array(image, dtype=float)
...
def CoM_Map(dat, MaxValue):
    ...
    temp_fig = np.zeros([Data_shape[2],Data_shape[3]])
    for i in range(Data_shape[0]):
        for j in range(Data_shape[1]):
            temp_fig = dat[i,j].copy()
            # get the max value of current image
            max_current = np.max(temp_fig)*MaxValue 
            # set all values in current image to be the max of current image
